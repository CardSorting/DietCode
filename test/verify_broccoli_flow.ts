...
async function verify() {
    ...
    const mockScanner: IntegrityScanner = {
        scan: async () => ({ score: 100, violations: [], scannedAt: new Date().toISOString(), fileCount: 10 }),
    ...
    fs.writeFileSync(path.resolve(projectRoot, oldPath), '/** [LAYER: INFRASTRUCTURE] */\nexport const temp = 1;');
    ...
    try {
        ...
        // Cleanup
        if (fs.existsSync(path.resolve(projectRoot, oldPath))) fs.unlinkSync(path.resolve(projectRoot, oldPath));
        if (fs.existsSync(path.resolve(projectRoot, newPath))) fs.unlinkSync(path.resolve(projectRoot, newPath));
