...
async function test() {
    ...
    const files = getAllTsFiles(path.join(projectRoot, 'src')).map(f => path.relative(projectRoot, f));
    ...
    await db.insertInto('sovereign_tasks' as any)
        .values({
            id: taskId,
            type: 'UNIFIED_SCAN',
            status: 'running',
    ...
    while (Date.now() - start < timeout) {
        const resultsCount = await db.selectFrom('job_results' as any)
    ...
    const jobResults = await db.selectFrom('job_results' as any)
        .selectAll()
        .where('taskId', '=', taskId)
        .execute() as any[];
    ...
    if (finalReport.violations.length > 0) {
        console.log('\nTop 5 Violations:');
        console.table(finalReport.violations.slice(0, 5).map(v => ({
            Type: v.type,
            File: v.file,
