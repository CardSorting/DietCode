...
class MockLogService implements LogService {
  log(message: string) {
    console.log(`[VAPOR_LOG] ${message}`);
  }
  error(message: string) {
    console.error(`[AETHER_ERR] ${message}`);
  }
  warn(message: string) {
    console.warn(`[MIDNIGHT_WARN] ${message}`);
  }
  debug(message: string) {
    console.debug(`[DREAM_DBG] ${message}`);
  }
  info(message: string) {
...
async function verify() {
  ...
  await CinematicRenderer.smoothFlow(
    COLORS.NEON_PINK('Streaming aether waves into the console...'),
    15,
  );
