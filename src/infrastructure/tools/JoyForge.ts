...
export class JoyForge {
    ...
    async extractInterface(filePath: string, projectRoot: string): Promise<ForgeResult> {
        ...
        const visit = (node: ts.Node) => {
            ...
            if (ts.isMethodDeclaration(node) && node.name) {
                ...
                if (!isPrivate && !isProtected) {
                    ...
                    methodSignatures.push(`  ${methodName}(${parameters}): Promise<${returnType}> | ${returnType};`);
                }
            }
        ...
        const interfaceName = className.startsWith('Sqlite') || className.startsWith('FileSystem') 
