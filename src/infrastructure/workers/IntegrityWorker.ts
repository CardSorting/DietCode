 ...
 */
parentPort.on('message', async (task: { filePath?: string, projectRoot: string, type: 'FILE' | 'PROJECT' }) => {
    ...
    try {
        if (type === 'FILE' && filePath) {
            ...
            parentPort!.postMessage({ success: true, message: 'Batch scan requested (not yet sharded in worker)' });
        }
    } catch (err: any) {
