...
export class SafetyEvaluator implements RiskEvaluator {
  ...
  async evaluateRisk(criteria: ActionCriteria): Promise<RiskLevel> {
    ...
    } else if (reversibility === 'LOCAL' || (affectedArea === 'SHARED' && systemImpact <= 6)) {
      return RiskLevel.LOW;
    } else if (reversibility === 'REMOTE' || affectedArea === 'EXTERNAL' || criteria.isDestructive) {
   ...
   */
  private determineAffectedArea(criteria: ActionCriteria): 'LOCAL' | 'SHARED' | 'EXTERNAL' | 'USERS' {
