...
export class PromptLoader {
  ...
  private classifyByMetadata(frontmatter: any, filepath: string): PromptCategory {
    ...
    if (contentLower.includes('tool') || contentLower.includes('shell') || contentLower.includes('file')) {
      return PromptCategory.TOOL_PROTOCOLS;
    }
    if (contentLower.includes('verify') || contentLower.includes('security') || contentLower.includes('risk')) {
  ...
  private applyDietCodeMetadata(frontmatter: any): Record<string, any> {
    ...
    if (frontmatter.memory_depth) {
      metadata.recommendedMemoryDepth = parseInt(frontmatter.memory_depth);
    }
