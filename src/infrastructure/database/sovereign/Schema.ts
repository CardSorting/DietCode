...
export class Schema {
  ...
  private static readonly TABLE_SCHEMAS: Record<string, string> = {
    ...
    knowledge_edges:
      'id TEXT PRIMARY KEY, sourceId TEXT NOT NULL, targetId TEXT NOT NULL, type TEXT NOT NULL, weight REAL DEFAULT 1.0',
    branches:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, name TEXT NOT NULL, head TEXT, isEphemeral INTEGER, createdAt INTEGER, expiresAt INTEGER',
    claims:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, branch TEXT NOT NULL, path TEXT NOT NULL, author TEXT NOT NULL, timestamp INTEGER NOT NULL, expiresAt INTEGER',
    ...
    files:
      'id TEXT PRIMARY KEY, path TEXT NOT NULL, content TEXT NOT NULL, encoding TEXT NOT NULL, size INTEGER NOT NULL, updatedAt BIGINT NOT NULL, author TEXT NOT NULL',
    reflog:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, ref TEXT NOT NULL, oldHead TEXT, newHead TEXT NOT NULL, author TEXT NOT NULL, message TEXT NOT NULL, timestamp BIGINT NOT NULL, operation TEXT NOT NULL',
    stashes:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, branch TEXT NOT NULL, nodeId TEXT NOT NULL, data TEXT NOT NULL, tree TEXT NOT NULL, label TEXT NOT NULL, createdAt BIGINT NOT NULL',
    knowledge:
      'id TEXT PRIMARY KEY, userId TEXT NOT NULL, type TEXT NOT NULL, content TEXT NOT NULL, tags TEXT, edges TEXT, inboundEdges TEXT, embedding TEXT, confidence REAL, hubScore INTEGER, expiresAt BIGINT, metadata TEXT, createdAt BIGINT',
    audit_events:
      'id TEXT PRIMARY KEY, userId TEXT NOT NULL, agentId TEXT, type TEXT NOT NULL, data TEXT NOT NULL, createdAt BIGINT NOT NULL',
    logical_constraints:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, pathPattern TEXT NOT NULL, knowledgeId TEXT NOT NULL, severity TEXT NOT NULL, createdAt BIGINT NOT NULL',
    decisions:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, agentId TEXT NOT NULL, taskId TEXT, decision TEXT NOT NULL, rationale TEXT NOT NULL, knowledgeIds TEXT NOT NULL, timestamp BIGINT NOT NULL',
    queue_jobs:
      'id TEXT PRIMARY KEY, payload TEXT NOT NULL, status TEXT NOT NULL, priority INTEGER DEFAULT 0, attempts INTEGER DEFAULT 0, maxAttempts INTEGER DEFAULT 5, runAt BIGINT, error TEXT, createdAt BIGINT, updatedAt BIGINT',
    tasks:
      'id TEXT PRIMARY KEY, userId TEXT NOT NULL, agentId TEXT NOT NULL, status TEXT NOT NULL, description TEXT NOT NULL, complexity REAL, linkedKnowledgeIds TEXT, result TEXT, createdAt BIGINT, updatedAt BIGINT',
    telemetry:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, agentId TEXT NOT NULL, taskId TEXT, promptTokens INTEGER NOT NULL, completionTokens INTEGER NOT NULL, totalTokens INTEGER NOT NULL, modelId TEXT NOT NULL, cost REAL NOT NULL, timestamp BIGINT NOT NULL, environment TEXT NOT NULL',
    telemetry_aggregates:
      'id TEXT PRIMARY KEY, repoPath TEXT NOT NULL, agentId TEXT NOT NULL, periodStart BIGINT NOT NULL, periodEnd BIGINT NOT NULL, totalPrompts INTEGER NOT NULL, totalCompletions INTEGER NOT NULL, totalTokens INTEGER NOT NULL, totalCost REAL NOT NULL',
    agents:
      'id TEXT PRIMARY KEY, userId TEXT NOT NULL, name TEXT NOT NULL, status TEXT NOT NULL, capabilities TEXT, metadata TEXT, createdAt BIGINT, updatedAt BIGINT',
    users:
      'id TEXT PRIMARY KEY, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, password TEXT, provider TEXT, providerId TEXT, avatarUrl TEXT, createdAt BIGINT, updatedAt BIGINT',
    workspaces:
      'id TEXT PRIMARY KEY, userId TEXT NOT NULL, sharedMemoryLayer TEXT, createdAt BIGINT',
    repositories:
   ...
   */
  private static async patchSingleTable(
    db: any,
    tableName: string,
    timeoutMs = 15000,
  ): Promise<boolean> {
    ...
    try {
      console.log(
        `[NUCLEAR PATCH] 🔍 Checking table '${tableName}' for standalone 'id' PRIMARY KEY...`,
      );

      // STEP 1: Fast schema check
      const result = await Schema.withTimeout(
      ...
      if (Array.isArray(result)) {
        ...
        tableInfo =
          result.rows || result.result || result.columns || result.data || Object.values(result);
      } else {
        throw new Error(
          `Cannot handle result type: ${typeof result}, value: ${JSON.stringify(result)}`,
        );
      }
      ...
      if (hasStandaloneIdPk) {
        ...
        console.log(
          `[NUCLEAR PATCH] ✓ Table '${tableName}' already has standalone 'id' PRIMARY KEY (skipped in ${elapsed}s)`,
        );
        return false;
      }
      ...
      if (!columnNames.has('id')) {
        ...
        console.log(
          `[NUCLEAR PATCH] 🐛 Table '${tableName}' has 'id' but missing proper PRIMARY KEY constraint - starting patch`,
        );
      }

      const targetSchema = Schema.TABLE_SCHEMAS[tableName];
      ...
      console.log(
        `[NUCLEAR PATCH] 📝 Creating temporary table '${tableName}_new' with proper schema`,
      );

      // STEP 2: Begin transaction
      await Schema.withTimeout(
      ...
      const columnDefinitions = (() => {
        ...
        if (tableName === 'logical_constraints') {
        ...
        if (tableName === 'telemetry_aggregates') {
          return [
            'id',
            'repoPath',
            'agentId',
      ...
      let rowCount = 0;
      const dataResult = await Schema.withTimeout(
      ...
      await Schema.withTimeout(
        Schema.withDbExecute(
          db,
          CompiledQuery.raw(`DELETE FROM ${tableName}_new WHERE id = '${idValue}'`),
        ),
        Math.min(timeoutMs, 500),
        'Delete template row',
      );
      ...
      console.log(
        `[NUCLEAR PATCH] 🔄 Swapping tables: DROP '${tableName}', RENAME '${tableName}_new' to '${tableName}'`,
      );

      // Verify data integrity
      const originalCountResult = await Schema.withTimeout(
      ...
      const newCountResult = await Schema.withTimeout(
      ...
      if (originalRowCount !== newRowCount) {
        throw new Error(
          `Data mismatch: ${originalRowCount} rows in original but ${newRowCount} in new table`,
        );
      }
      ...
      const patchedTableInfo = await Schema.withTimeout(
        db.executeQuery(CompiledQuery.raw(`PRAGMA table_info(${tableName})`)),
        Math.min(timeoutMs, 2000),
      ...
      if (Array.isArray(patchedTableInfo)) {
        ...
        patchedColumns =
          patchedTableInfo.rows || patchedTableInfo.data || Object.values(patchedTableInfo);
      } else {
        patchedColumns = [];
      }

      if (
        !Array.isArray(patchedColumns) ||
        !new Set(patchedColumns.map((col: any) => col.name)).has('id')
      ) {
      ...
      console.log(
        `[NUCLEAR PATCH] ✓ Table '${tableName}' SUCCESSFULLY PATCHED with 'id' in ${elapsedTime}s`,
      );
      return true;
    } catch (error: any) {
      ...
      const elapsedTime = ((Date.now() - startTime) / 1000).toFixed(2);
      console.error(
        `[NUCLEAR PATCH] ❌ FAILED to patch table '${tableName}' after ${elapsedTime}s:`,
        error.message,
      );
      throw error;
    }
  ...
  private static async withTimeout(
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => {
        reject(
          new Error(`[NUCLEAR PATCH] ⚠️  TIMEOUT after ${timeoutMs}ms on operation: ${operation}`),
        );
      }, timeoutMs);
    });
  ...
  static async ensureSchema(dbPool: any) {
    ...
    for (const shardId of shards) {
      ...
      for (const tableName of Schema.SYSTEM_TABLES) {
        const tableStart = Date.now();
        try {
          const patched = await Schema.patchSingleTable(db, tableName);
          if (patched) {
            totalSuccessCount++;
          } else {
            totalSkipCount++;
          ...
          totalFailCount++;
          console.error(
            `[NUCLEAR PATCH] ⚠️  Table '${tableName}' failed on shard '${shardId}', continuing...`,
          );
    ...
    console.log(
      `[NUCLEAR PATCH] 📊 Total Summary: ${totalSuccessCount} patched, ${totalSkipCount} skipped, ${totalFailCount} failed across all shards`,
    );

    if (totalFailCount > 0) {
      console.warn(
        `[NUCLEAR PATCH] ⚠️  ${totalFailCount} table(s) failed patching. Deployment may be unstable.`,
      );
    }
  }
}
