...
async function runRigorousTest() {
  ...
  const qsData = await db.executeQuery(CompiledQuery.raw('SELECT * FROM queue_settings ORDER BY key ASC'));
  ...
  if (qsRows[0].key !== 'test_key_1' || qsRows[0].value !== 'value_1') throw new Error('Data corruption in queue_settings');
  ...
  const brData = await db.executeQuery(CompiledQuery.raw('SELECT * FROM branches ORDER BY name ASC'));
  ...
  if (!setColumns.includes('updatedAt')) throw new Error('settings missing updatedAt column after patch');
