...
export class VitalsDashboard {
  ...
  public render(heartbeat: VitalsHeartbeat): void {
    ...
    // Focus Shield Status
    console.log(`Focus Shield  : ${shield.active ? '\x1b[32mACTIVE\x1b[0m' : '\x1b[33mINACTIVE\x1b[0m'} (${shield.allowedCount} allowed paths)`);
    
    console.log('-------------------------------------------\n');
  }

  private printMetric(label: string, value: number, warn: number, error: number, lowerIsBetter: boolean = false): void {
    let color = '\x1b[32m'; // Green
    
    if (lowerIsBetter) {
        if (value > error) color = '\x1b[31m'; // Red
