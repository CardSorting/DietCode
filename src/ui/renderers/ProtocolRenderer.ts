 ...
 */
import { BORDERS, COLORS, ICONS, SYMBOLS, WAVEFORMS } from '../design/Theme';
...
export const ProtocolRenderer = {
  ...
  renderStepHeader(step: number, total: number, title: string, _project?: string): string {
    ...
    const bar =
      SYMBOLS.FULL_BLOCK.repeat(Math.max(0, filled - 1)) +
      (filled > 0 ? spark : '') +
  ...
  renderConnectivityStatus(
    providers: { name: string; status: 'CONNECTED' | 'PENDING' | 'MISSING' | 'ERROR' }[],
  ): string {
    ...
    const box = [
      COLORS.PRIMARY(
        `${BORDERS.tl}${BORDERS.h} AI SERVICES CHECK ${BORDERS.h.repeat(22)}${BORDERS.tr}`,
      ),
      ...lines.map(
        (l) => `${COLORS.PRIMARY(BORDERS.v)} ${l.padEnd(38)} ${COLORS.PRIMARY(BORDERS.v)}`,
      ),
  ...
  async renderWaveformPulse(intensity: number): Promise<void> {
    ...
    process.stdout.write(
      `\r ${COLORS.PRIMARY('SIGNAL:')} ${wave} [ ${COLORS.HIVE_GOLD(`${Math.floor(intensity * 100)}%`)} ] `,
    );
  },
  ...
  async renderHandshakePulse(provider: string): Promise<void> {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      process.stdout.write(
        `\r  ${COLORS.MUTED('[')} ${COLORS.lerpHeat(frame ?? '', intensity)} ${COLORS.MUTED(']')} SYNCHRONIZING... ${Math.floor(intensity * 100)}%`,
      );
