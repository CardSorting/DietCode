...
export class SovereignSelector {
  ...
  async generateProvenanceBundle(task: TaskEntity): Promise<ProvenanceBundle> {
    ...
    const isCoreTarget =
      objective.includes('domain') ||
      objective.includes('infrastructure') ||
      objective.includes('core');
    ...
    const reqsWithVerification = task.requirements.filter(
      (r) => r.verificationCriteria && r.verificationCriteria.length > 0,
    ).length;
