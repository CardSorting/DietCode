...
describe("Remote Config Schema", () => {
  ...
  describe("RemoteMCPServersSchema", () => {
    it("should accept remoteMCPServers with alwaysEnabled true", () => {
      const config = {
        version: "v1",
        remoteMCPServers: [
          { name: "always-on-server", url: "https://example.com/mcp", alwaysEnabled: true },
        ],
      };
    ...
    it("should accept remoteMCPServers with alwaysEnabled false", () => {
      const config = {
        version: "v1",
        remoteMCPServers: [
          { name: "toggle-server", url: "https://example.com/mcp", alwaysEnabled: false },
        ],
      };
    ...
    it("should reject remoteMCPServers with invalid alwaysEnabled type", () => {
      const config = {
        version: "v1",
        remoteMCPServers: [
          { name: "test-server", url: "https://example.com/mcp", alwaysEnabled: "yes" },
        ],
      };
  ...
  describe("RemoteConfigSchema", () => {
    ...
    it("should handle complete config with all fields", () => {
      ...
      expect(result.providerSettings?.OpenAiCompatible?.azureApiVersion).to.equal(
        "2024-02-15-preview",
      );
      ...
      expect(result.enterpriseTelemetry?.promptUploading?.s3AccessSettings?.bucket).to.equal(
        "enterprise-prompts",
      );
      expect(result.enterpriseTelemetry?.promptUploading?.s3AccessSettings?.region).to.equal(
        "us-east-1",
      );
    });
  });
