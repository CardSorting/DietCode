...
async function main() {
  ...
  checks.push(await runCheck(
    'No "any" types in tools directory',
    async () => {
      ...
      return {
        passed: !hasAny,
        message: hasAny 
          ? 'Found "any" type usage in tools directory' 
          : 'No "any" types found',
        severity: 'CRITICAL'
      };
    }
  ...
  checks.push(await runCheck(
    ...
    async () => {
      ...
      const hasValidation = parseInt(fileToolsCode) >= 1;
      return {
  ...
  checks.push(await runCheck(
    'Grep includes recursion depth limit',
    async () => {
      const grepCode = execSync(
        'cat src/infrastructure/tools/grep.ts | grep -c "MAX_DEPTH"',
        { encoding: 'utf8' }
      );
      const hasLimit = parseInt(grepCode) >= 1;
      return {
        passed: hasLimit,
        message: hasLimit 
  ...
  checks.push(await runCheck(
    'Grep includes forbidden path checks',
    async () => {
      const grepCode = execSync(
        'cat src/infrastructure/tools/grep.ts | grep -c "FORBIDDEN_PATHS"',
        { encoding: 'utf8' }
      );
      const hasCheck = parseInt(grepCode) >= 1;
      return {
        passed: hasCheck,
        message: hasCheck 
  ...
  checks.push(await runCheck(
    'Mkdir includes path validation',
    async () => {
      const mkdirCode = execSync(
        'cat src/infrastructure/tools/mkdir.ts | grep -c "validatePath"',
        { encoding: 'utf8' }
      );
      const hasValidation = parseInt(mkdirCode) >= 1;
      return {
        passed: hasValidation,
        message: hasValidation 
  ...
  checks.push(await runCheck(
    ...
    async () => {
      ...
      const hasLimit = parseInt(fileToolsCode) >= 1;
      return {
        passed: hasLimit,
        message: hasLimit 
  ...
  checks.push(await runCheck(
    'Directory traversal has depth limit',
    async () => {
      const grepCode = execSync(
        'cat src/infrastructure/tools/grep.ts | grep -c "MAX_DEPTH"',
        { encoding: 'utf8' }
  ...
  checks.push(await runCheck(
    'Search results have maximum limit',
    async () => {
      const grepCode = execSync(
  ...
  checks.push(await runCheck(
    'All tools handle unknown errors',
    async () => {
      const grepCode = execSync(
        'cat src/infrastructure/tools/grep.ts | grep -c "String(error)"',
        { encoding: 'utf8' }
      );
      const hasHandling = parseInt(grepCode) >= 1;
      return {
        passed: hasHandling,
        message: hasHandling 
  ...
  checks.push(await runCheck(
    'All tools use proper type casts',
    async () => {
      const grepCode = execSync(
  ...
  checks.push(await runCheck(
    'Tools import Domain contracts',
    async () => {
      const toolsDir = execSync(
        'ls -1 src/infrastructure/tools/*.ts | xargs -I {} basename {}',
        { encoding: 'utf8' }
      );
      const files = toolsDir.split('\n').filter(Boolean);
      const domainImports = files.filter(file => 
        execSync(`head -20 src/infrastructure/tools/${file} | grep "import.*from.*domain/agent"`, { encoding: 'utf8' }).length > 0
      );
      return {
