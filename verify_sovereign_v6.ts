...
import { TaskState, TaskPriority, EntityState } from './src/domain/task/TaskEntity';
...
async function verifySovereignHardening() {
  ...
  const testTask: any = {
    ...
    requirements: [
      { uniqueId: 'harden-01', description: 'Deep audit of the system', type: 'refactor', priority: TaskPriority.HIGH, isCritical: true, section: 'Verification' }
    ],
  ...
  const dbResult = await db.selectFrom('metabolic_telemetry' as any)
    .selectAll()
  ...
  if (!dbResult) {
    throw new Error('❌ Database Persistence ERROR: Metabolic data not found in metabolic_telemetry table.');
  }
  ...
  await scheduler.simulateShadowExecution(task, fileA, '// [LAYER: DOMAIN]\nexport const A = 1;', process.cwd(), {});
  const end = Date.now();
