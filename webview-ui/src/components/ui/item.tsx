  ...
  'group/item [a]:hover:bg-accent/50 focus-visible:border-ring focus-visible:ring-ring/50 flex flex-wrap items-center rounded-sm border border-transparent text-sm outline-none focus-visible:ring-[3px]',
  {
    variants: {
      variant: {
        ...
        select:
          'bg-input-background/50 hover:bg-input-background/70 border border-input-foreground/10',
        muted: 'bg-muted/50',
      },
      size: {
        default: 'gap-1 p-2 ',
...
function ItemContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'w-full flex flex-1 flex-col gap-1 [&+[data-slot=item-content]]:flex-none',
        className,
      )}
