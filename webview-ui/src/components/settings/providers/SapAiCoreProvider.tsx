 ...
 */
export const SapAiCoreProvider = ({
  showModelOptions,
  isPopup,
  currentMode,
}: SapAiCoreProviderProps) => {
  ...
  const [sapAiCoreModelDeployments, setSapAiCoreModelDeployments] = useState<
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {orchestrationAvailable && (
        <div className="flex flex-col gap-2.5 mt-[15px]">
          ...
          <p className="text-xs text-(--vscode-descriptionForeground)">
            When enabled, provides access to all available models without requiring individual
            deployments.
            <br />
            <br />
              ...
              <>
                {sapAiCoreModelDeployments.length === 0 && (
                  <div className="text-xs text-(--vscode-errorForeground) mb-2">
                    Unable to fetch models from SAP AI Core service instance. Please check your SAP
                    AI Core configuration or ensure your deployments are deployed and running in the
                    service instance
                  </div>
                )}
