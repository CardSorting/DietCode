...
const VercelModelPicker: React.FC<VercelModelPickerProps> = ({ isPopup, currentMode }) => {
  ...
  const { apiConfiguration, vercelAiGatewayModels, refreshVercelAiGatewayModels } =
    useExtensionState();
      ...
      {hasInfo && selectedModelInfo ? (
        ...
        >
          {Object.keys(vercelAiGatewayModels).length === 0 ? (
            ...
            <>
              Select a model from the dropdown above. The extension fetches available models from
              your Vercel AI Gateway configuration.
            </>
          )}
        </p>
      )}
