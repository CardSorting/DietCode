...
export function SovereignTerminal({ command, output, isExecuting }: SovereignTerminalProps) {
  ...
  // Auto-scroll logic for terminal fidelity
  useEffect(() => {
  ...
  const formattedOutput = useMemo(() => {
    return output.split('\n').map((line, i) => {
      ...
      if (line.includes('[31m') || line.toLowerCase().includes('error')) className += ' term-error';
      if (line.includes('[32m') || line.toLowerCase().includes('success')) className += ' term-success';
      if (line.includes('[33m')) className += ' term-warn';
