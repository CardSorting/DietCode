...
export async function searchWorkspaceFilesMultiroot(
  ...
  try {
    ...
    if (workspaceHint) {
      const targetWorkspace = workspaceRoots.find(
        (root: WorkspaceRoot) => root.name === workspaceHint,
      );
      if (targetWorkspace) {
    ...
    const searchPromises = workspacesToSearch.map(async (workspace) => {
      try {
        ...
        Logger.error(
          `[searchWorkspaceFilesMultiroot] Error searching workspace ${workspace.name}:`,
          error,
        );
        return [];
      }
