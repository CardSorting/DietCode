...
class ClineOAuthClientProvider implements OAuthClientProvider {
  ...
  async clientInformation(): Promise<OAuthClientInformationFull | undefined> {
    ...
    if (!this.isRegistrationValid) {
      ...
      if (secrets[this.serverHash]?.client_info) {
        Logger.log(
          `[McpOAuth] Discarding stale client registration for ${this.serverName} — redirect URL changed`,
        );
        // Clear the stale client_info and tokens (tokens are bound to the old client_id)
        secrets[this.serverHash].client_info = undefined;
        secrets[this.serverHash].redirect_url_at_registration = undefined;
        secrets[this.serverHash].tokens = undefined;
        secrets[this.serverHash].tokens_saved_at = undefined;
  ...
  async redirectToAuthorization(authorizationUrl: URL): Promise<void> {
    ...
    Logger.log(
      `[McpOAuth] OAuth required for ${this.serverName} - user must click Authenticate button`,
    );
  }
  ...
  validateAndClearState(serverHash: string, state: string): boolean {
    ...
    if (serverData.oauth_state_timestamp) {
      if (Date.now() - serverData.oauth_state_timestamp > this.STATE_EXPIRY_MS) {
        ...
        // Clear expired state
        serverData.oauth_state = undefined;
        serverData.oauth_state_timestamp = undefined;
        saveMcpOAuthSecrets(secrets);
        return false;
      }
    }
