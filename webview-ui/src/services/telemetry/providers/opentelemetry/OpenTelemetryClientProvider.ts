...
export class OpenTelemetryClientProvider {
  ...
  constructor(config: OpenTelemetryClientValidConfig) {
    ...
    if (isDebugMode) {
      ...
      Logger.log(
        `[OTEL DEBUG]   - Metric Export Interval: ${this.config.metricExportInterval || 60000}ms`,
      );
    }
    ...
    for (const exporterType of exporters) {
      try {
        switch (exporterType) {
          ...
          case "otlp": {
            const protocol =
              this.config?.otlpMetricsProtocol || this.config?.otlpProtocol || "grpc";
            ...
            if (endpoint) {
              const reader = createOTLPMetricReader(
                protocol,
                endpoint,
                insecure,
                interval,
              ...
              if (reader) {
                readers.push(reader);
                Logger.log(
                  `[OTEL] OTLP metrics reader created (${protocol}, interval: ${interval}ms)`,
                );
              }
            } else {
