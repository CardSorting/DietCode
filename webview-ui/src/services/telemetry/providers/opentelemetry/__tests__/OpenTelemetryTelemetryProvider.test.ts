...
describe('OpenTelemetryTelemetryProvider.identifyUser', () => {
  ...
  it('should emit user_identified log and update distinct ID when ID changes', () => {
    ...
    expect(setDistinctIdStub.calledOnceWith('user-1'), 'Should call setDistinctId with user ID').to
      .be.true;
  });
  ...
  it('should refresh userAttributes even when distinct ID already matches (no new identify log)', () => {
    ...
    expect(setDistinctIdStub.notCalled, 'Should not call setDistinctId when ID already matches').to
      .be.true;
    ...
    expect(attrs.organization_id, 'organization_id should be present in subsequent logs').to.equal(
      'org-a',
    );
  ...
  it('should refresh org attributes when active org changes (same user ID)', () => {
    ...
    expect(records.length, 'Should not emit identify log on org switch with same user ID').to.equal(
      0,
    );
    ...
    expect(records[0].attributes.organization_name, 'Should reflect new org name').to.equal(
      'Org B',
    );
  ...
  it('should clear stale org attributes when switching from org to no-org', () => {
    ...
    records = logExporter.getFinishedLogRecords();
    expect(records.length).to.equal(1);
    expect(records[0].attributes.organization_id, 'Stale org_id should be cleared').to.equal(
      undefined,
    );
    expect(records[0].attributes.organization_name, 'Stale org_name should be cleared').to.equal(
      undefined,
    );
  });
