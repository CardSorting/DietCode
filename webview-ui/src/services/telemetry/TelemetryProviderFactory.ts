    ...
    switch (config.type) {
      ...
      case "opentelemetry": {
        ...
        if (client.meterProvider || client.loggerProvider) {
          return await new OpenTelemetryTelemetryProvider(
            client.meterProvider,
            client.loggerProvider,
            {
...
export class NoOpTelemetryProvider implements ITelemetryProvider {
  ...
  identifyUser(_userInfo: any, _properties?: TelemetryProperties): void {
    Logger.info(
      `[NoOpTelemetryProvider] identifyUser - ${JSON.stringify(_userInfo)} - ${JSON.stringify(_properties)}`,
    );
  }
  isEnabled(): boolean {
    return false;
