...
export class BannerService {
  ...
  private getCacheDurationMs(): number {
    const flagPayload = featureFlagsService.getFlagPayload(
  ...
  private async doFetch(): Promise<Banner[]> {
    ...
    try {
      ...
      Logger.log(
        `[BannerService] Raw API response: ${data.data.items.length} items: ${JSON.stringify(
          data.data.items.map((b) => ({
            ...
            titleMd: b.titleMd?.substring(0, 50),
          })),
        )}`,
      );
      ...
      Logger.log(
        `[BannerService] Fetched ${banners.length} banner(s) at ${new Date(this.lastFetchTime).toISOString()}`,
      );
      return banners;
    } catch (error) {
      clearTimeout(timeoutId);
  ...
  private toBannerCardData(banner: Banner): BannerCardData | null {
    ...
    for (const action of actions) {
      if (!action.action || !this.validActionTypes.has(action.action) || !action.title) {
        Logger.error(
          `[BannerService] Invalid action type (${action.action}) for banner ${banner.id}`,
        );
        return null;
      }
    }
