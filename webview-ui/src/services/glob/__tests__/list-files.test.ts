...
describe("listFiles", () => {
  const tmpDir = path.join(
    os.tmpdir(),
    `cline-list-files-test-${Math.random().toString(36).slice(2)}`,
  );
...
describe("listFiles gitignore handling", () => {
  ...
  const baseDir = path.join(
    os.tmpdir(),
    `cline-gitignore-test-${Math.random().toString(36).slice(2)}`,
  );
  ...
  it("excludes files matching root .gitignore directory patterns", async () => {
    ...
    hasIgnoredContent.should.equal(
      false,
      "ignored-dir/ contents should be excluded by root .gitignore",
    );
  });
  ...
  it("does not read .gitignore from inside gitignored directories", async () => {
    ...
    hasThirdPartyContent.should.equal(
      false,
      "third-party/ contents should be excluded — and its .gitignore files never read",
    );
  });
});
