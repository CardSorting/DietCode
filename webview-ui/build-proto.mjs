...
const PROTOC =
  isWindows && fsSync.existsSync(LEGACY_WINDOWS_PROTOC) ? LEGACY_WINDOWS_PROTOC : GRPC_TOOLS_PROTOC;
...
function checkAppleSiliconCompatibility() {
  ...
  if (cpuArchitecture === 'arm64') {
    try {
      ...
      const rosettaCheck = execSync('/usr/bin/pgrep oahd || echo "NOT_INSTALLED"')
        .toString()
        .trim();
      ...
      console.log(
        chalk.yellow('Could not determine Rosetta installation status. Proceeding anyway.'),
      );
    }
  }
}
