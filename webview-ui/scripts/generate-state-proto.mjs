...
function parseSecretsKeys(sourceFile) {
  ...
  if (initializer.getKind() !== SyntaxKind.ArrayLiteralExpression) {
    throw new Error(
      `SECRETS_KEYS is not an array literal (got ${SyntaxKind[initializer.getKind()]})`,
    );
  }
...
function parseProtoMessageFieldNumbers(protoContent, messageName) {
  ...
  const fieldRegex =
    /(?:optional|required|repeated)?\s*(?:map<[^>]+>|[\w.]+)\s+(\w+)\s*=\s*(\d+)\s*;/g;
...
function assignFieldNumbers(fields, existingNumbers, startNumber = 1) {
  ...
  for (const field of fields) {
    ...
    if (
      normalizedFieldNumber !== undefined &&
      rawFieldNumber !== undefined &&
      normalizedFieldNumber !== rawFieldNumber
    ) {
      throw new Error(
