...
async function runChild() {
    try {
        ...
        await AuditRecorder.recordAudit('CONCURRENCY_TEST', `Process ${process.pid} initialized successfully`);
        process.exit(0);
    } catch (err: any) {
...
async function runMain() {
    ...
    try {
        ...
        const logs = await (db as any).selectFrom('audit_log' as any).selectAll().execute();
