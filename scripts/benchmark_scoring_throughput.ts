...
async function runBenchmark() {
    ...
    const cachePromises = [];
    for (let i = 0; i < 20; i++) {
        cachePromises.push(proxy.executeSingle(JobType.SEMANTIC_SCORING, { content: testContent }, { priority: 1 }));
    }
    await Promise.all(cachePromises);
    const cacheDuration = Date.now() - cacheStart;
    console.log(`✅ Cache Test: 20 jobs completed in ${cacheDuration}ms (Avg: ${cacheDuration/20}ms)\n`);

    console.log('📊 Phase 2: Parallel Throughput Verification (20 unique jobs)');
    const throughputStart = Date.now();
    const throughputPromises = [];
    for (let i = 0; i < 20; i++) {
        throughputPromises.push(proxy.executeSingle(JobType.SEMANTIC_SCORING, { content: uniqueContent(i) }, { priority: 0 }));
    }
    await Promise.all(throughputPromises);
    const throughputDuration = Date.now() - throughputStart;
    console.log(`✅ Throughput Test: 20 unique jobs completed in ${throughputDuration}ms (Avg: ${throughputDuration/20}ms)\n`);

    console.log('📈 Benchmark Results Summary:');
    console.log(`   - Repeated Jobs (Cached): ${cacheDuration}ms`);
    console.log(`   - Unique Jobs (Computed): ${throughputDuration}ms`);
    console.log(`   - Cache Efficiency: ${((throughputDuration - cacheDuration) / throughputDuration * 100).toFixed(2)}% Improvement\n`);
